...
final class IdTokenEmailExtractorTest extends TestCase
{
    ...
    /**
     * A payload segment whose length is ≡ 1 (mod 4) cannot be valid base64 — no encoder ever emits
     * one — so the padding loop produces a string that base64_decode(..., true) rejects outright.
     * The decode therefore short-circuits before json_decode() is ever reached.
     */
    public function testExtract_returnsNullWhenThePayloadIsNotDecodableBase64(): void
    {
        $encode = static fn (string $raw): string => rtrim(strtr(base64_encode($raw), '+/', '-_'), '=');

        self::assertSame(1, \strlen('abcde') % 4);
        self::assertNull($this->extractor->extract($encode('{}') . '.abcde.sig'));
    }
