...
final class BancontactSupportedRefundPaymentMethodsProviderDecorator implements RefundPaymentMethodsProviderInterface
{
    ...
    public function findForChannel(ChannelInterface $channel): array
    {
        ...
        foreach ($paymentMethods as $key => $paymentMethod) {
            ...
            if (BancontactGatewayFactory::FACTORY_NAME !== $gatewayConfig->getFactoryName()) {
                continue;
            }
    ...
    private function isBancontact(OrderInterface $order): bool
    {
        $lastPayment = $order->getLastPayment(PaymentInterface::STATE_COMPLETED);
        if (!$lastPayment instanceof PaymentInterface) {
            return false;
