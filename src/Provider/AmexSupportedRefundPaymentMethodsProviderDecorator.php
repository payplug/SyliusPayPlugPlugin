...
final class AmexSupportedRefundPaymentMethodsProviderDecorator implements RefundPaymentMethodsProviderInterface
{
    ...
    public function findForChannel(ChannelInterface $channel): array
    {
        ...
        if ($this->isAmericanExpress($order)) {
        ...
        foreach ($paymentMethods as $key => $paymentMethod) {
            ...
            if (AmericanExpressGatewayFactory::FACTORY_NAME !== $gatewayConfig->getFactoryName()) {
                continue;
            }
    ...
    private function isAmericanExpress(OrderInterface $order): bool
    {
        $lastPayment = $order->getLastPayment(PaymentInterface::STATE_COMPLETED);
        if (!$lastPayment instanceof PaymentInterface) {
            return false;
