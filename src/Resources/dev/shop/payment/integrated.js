const IntegratedPayment = {
  ...
  load() {
    IntegratedPayment.options.api = integratedPaymentApi = new Payplug.IntegratedPayment(true);
    ...
    integratedPaymentApi.onValidateForm(async ({isFormValid}) => {
      if (isFormValid) {
        if (payplug_integrated_payment_params.payment_id !== undefined) {
          integratedPaymentApi.pay(payplug_integrated_payment_params.payment_id, 'visa');
          return;
        }
