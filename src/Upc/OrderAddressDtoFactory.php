 ...
 * in one place.
 */
final class OrderAddressDtoFactory
{
    ...
    private function buildAddress(AddressInterface $address): AddressDto
    {
        return new AddressDto(
            ...
            // AddressDto::$state is documented as 0-3 chars, so the short province CODE (e.g.
            // "75") fits where the full province NAME (used elsewhere in this plugin's legacy
            // PayPlugPaymentDataCreator) would not.
    ...
    {
        return match ($order->getCustomer()?->getGender()) {
            'm' => 'MR',
            'f' => 'MRS',
            default => null,
        };
